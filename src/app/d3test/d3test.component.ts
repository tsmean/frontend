  ...
  function wrap(text, width) {
    text.each(function() {
      const innerText = d3.select(this),
        words = innerText.text().split(/\s+/).reverse(),
        lineHeight = 1.4, //  ems
        y = innerText.attr('y'),
        x = innerText.attr('x'),
        dy = parseFloat(innerText.attr('dy'));
      let tspan = innerText.text(null).append('tspan').attr('x', x).attr('y', y).attr('dy', dy + 'em'),
        word,
        lineNumber = 0,
        line = [];
      ...
      while (word = words.pop()) {
        ...
        if (tspan.node().getComputedTextLength() > width) {
          ...
          tspan.text(line.join(' '));
          line = [word];
          tspan = innerText.append('tspan').attr('x', x).attr('y', y).attr('dy', ++lineNumber * lineHeight + dy + 'em').text(word);
        }
      }
    });
  }// wrap
